	...
	public class PengurusanKakitanganSambilanController : Controller
	{
		...
		public ActionResult PDFSenaraiPergerakanGaji(string jenis, int? bulan, int? tahun, string Command, string gajibonus, string sambilan, int? pk)
		{
			...
			if (jenis == "10") //perkeso sambilan
			{
                ...
                {
                    string longTextfile = GetTextFile();
                    return File(Encoding.UTF8.GetBytes(longTextfile), "text/plain", "sampleText.txt");
                }
			}
        ...
        private string GetTextFile()
        {
            ...
            for(int i = 0; i < 7; i++)
            {
                string line = string.Format("{0}\t{1}\t{2}", "B3200000538V", "940920036250", "TUAN NAZURAH BINTI TUAN MUT");
